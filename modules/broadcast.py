...
async def gcast(event):
    if xx := event.pattern_match.group(1):
        ...
        return await eor(
            event, "`Kasi pesan dulu lah tolol`"
        )
    kk = await event.eor("`SABAR MEMEK LAGI NGIRIM PAKET AWAS LIMIT NTAR NANGIS🤪!!...`")
    er = 0
    done = 0
    err = ""
        ...
        if x.is_group:
            ...
            if chat not in chat_blacklist and chat not in NOSPAM_CHAT:
                try:
                    ...
                    er += 1
    await kk.edit(
        f"**Berhasil Menghamili : `{done}` Anak.\nKeguguran Sebanyak : `{er}` Anak.**"
    )
        ...
        if x.is_user and not x.entity.bot:
            ...
            if chat not in DEVS and chat not in chat_blacklist:
                try:
                    ...
                    er += 1
    await kk.edit(
        f"**Lumayan Nyolong Rokok Sebanyak : `{done}` Warung Madura.\nAnjing Ketinggalan Sebanyak : `{er}` Indomaret.**"
    )
...
async def gblacker(event, type_):
    ...
    if type_ == "add":
        ...
        await event.eor(f"**Ditambahkan ke dalam Pacar Haha🗿 Blacklist Gcast**\n`{chat_id}`")
    elif type_ == "remove":
        rem_gblacklist(chat_id)
        await event.eor(f"**Dihapus dari Blacklist Gcast Maniez**\n`{chat_id}`")
